...
app.post('/api/webhook', async (req, res) => {
  ...
  try {
    const auth = new GitHubAppAuth(process.env.GITHUB_APP_ID, './private-key.pem');
    ...
    const depFindings = await scanDependencies(octokit, repo.owner.login, repo.name, pr.number, pr.head.sha);
    ...
    if (allFindings.length > 0) {
      let commentBody = '## 🛡️ GitSentry Security Findings\n\n';
      
      const secretFindings = allFindings.filter(f => f.category === 'secret');
      const depFindingsOnly = allFindings.filter(f => f.category === 'dependency');
      ...
      if (depFindingsOnly.length > 0) {
        ...
        depFindingsOnly.forEach((f, idx) => {
          const severityEmoji = f.severity === 'critical' || f.severity === 'high' ? '🔴' : 
                                f.severity === 'medium' ? '🟡' : '🟢';
          commentBody += `${severityEmoji} **${f.package}** (${f.severity})\n`;
          commentBody += `   - ${f.title}\n`;
          if (f.cve && f.cve !== 'N/A') commentBody += `   - CVE: ${f.cve}\n`;
          if (f.vulnerable_versions) commentBody += `   - Affected: ${f.vulnerable_versions}\n`;
